...
.controller('recipeControllerBm', function($scope, $stateParams, recipeServices, InputTopRecipe, $ionicLoading, $ionicModal, CacheFactory, FavouriteRecipe, $state, FilterCuisine, FilterOccasion, ionicToast) {
 	$scope.mySelect = InputTopRecipe;
 	$scope.filterCuisine = FilterCuisine;
 	$scope.filterOccasion = FilterOccasion;
	...
	$scope.getBestRecipe = function getBestRecipe(){
		$ionicLoading.show();
        recipeServices.getBestRecipeBm({
            topRecipe: $scope.mySelect,
            cuisine: $scope.filterCuisine,
            occasion: $scope.filterOccasion
        }).success(function(data){
...
.controller('findRecipeController', function($scope, $state, findRecipeServices, InputCuisine, InputOccasion, InputIngredient, InputSearch, $ionicLoading, ionicToast) {
    ...
    $scope.findRecipe = function findRecipe(userSearch){
        ...
        findRecipeServices.findRecipe({
            ...
            if($scope.foundRecipe.length == 0){
            	$scope.recipeNotFound = true;
            }else {
            	$scope.recipeNotFound = false;
            }
            $ionicLoading.hide();
...
.controller('findRecipeControllerBm', function($scope, $state, findRecipeServices, InputCuisineBm, InputOccasionBm, InputIngredientBm, InputSearchBm, $ionicLoading, ionicToast) {
 	// $scope.userCuisine=[];
 	$scope.userCuisine = InputCuisineBm;
 	$scope.userOccasion = InputOccasionBm;
 	$scope.ingredient = InputIngredientBm;
    ...
    $scope.findRecipe = function findRecipe(userSearch){
    	...
    	$ionicLoading.show();
        findRecipeServices.findRecipeBm({
            cuisine: $scope.userCuisine,
            occasion: $scope.userOccasion,
            ingredient: $scope.ingredient.name,
            search: userSearch
    ...
    $scope.findRecipe(InputSearchBm);
})
